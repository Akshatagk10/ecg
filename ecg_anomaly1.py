...
def plot_residual_with_lime(data, index):
    ...
    residuals = data[index].numpy() - dec_img[index].numpy()
    ...
    for idx, (feature, weight) in enumerate(lime_weights.items()):
        match = re.search(r'Feature (\d+)', feature)
        if match:
            feature_idx = int(match.group(1))
            if 0 <= feature_idx < len(residuals):
                ax.scatter(feature_idx, residuals[feature_idx], color='blue', s=abs(weight) * 100, alpha=0.6, label='LIME Importance' if idx == 0 else "")
